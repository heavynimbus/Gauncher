...
public class CommandHandler extends SimpleHandler {
  ...
  private void enterInChannel(String channelName) {
    if (ServerV1.channelList.stream().map(Channel::getName).anyMatch(channelName::equals)) {
      var notFullChannels =
          ServerV1.channelList.stream()
              .filter(channel -> !channel.isFull())
              .filter(channel -> channel.getName().equalsIgnoreCase(channelName))
              .collect(Collectors.toList());
      ServerV1.channelList.stream().filter(channel -> !channel.isFull()).forEach(System.out::println);
